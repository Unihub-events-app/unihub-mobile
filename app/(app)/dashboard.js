...
function EventListRow({ event, theme, forYou }) {
  ...
  const cardBg =
    theme.mode === "dark" ? "rgba(44,44,38,0.2)" : "rgba(255,255,255,0.2)";
      ...
      >
        ...
        >
          ...
          <View style={styles(theme).listThumb}>
            ...
            {forYou && <View style={styles(theme).forYouDot} />}
          </View>

          {/* Title + venue below image */}
          <Text
            style={[styles(theme).listTitle, { marginTop: 10 }]}
            numberOfLines={1}
          >
            {event.name}
          </Text>
          ...
          >
            ...
            <Text style={styles(theme).listVenue} numberOfLines={1}>
              {event.venue}
            </Text>
          </View>

          {/* Date / time + price on same row */}
          <View
            style={{
              flexDirection: "row",
              alignItems: "center",
          ...
          >
            {dm ? (
              <View style={styles(theme).dateBadge}>
                <Text style={styles(theme).dateBadgeText}>
                  {dm.day} {dm.month}
                </Text>
              </View>
            ) : null}
            ...
            >
              ...
              >
                {isFree
                  ? "Free"
                  : `₦${parseInt(event.price || 0).toLocaleString()}`}
              </Text>
            </View>
          </View>
        </Pressable>
  ...
  return (
    <Animated.View style={[styles(theme).heroWrap, animStyle]}>
      <Pressable
        onPress={() => router.push(`/event/${event.event_id}`)}
        onPressIn={() => {
          scale.value = withSpring(0.97, springs.smooth);
        }}
        onPressOut={() => {
          scale.value = withSpring(1.0, springs.smooth);
        }}
      ...
      >
        ...
        <View style={styles(theme).heroContent}>
          ...
          <Text style={styles(theme).heroTitle} numberOfLines={2}>
            {event.name}
          </Text>
          <View style={styles(theme).heroMeta}>
            ...
            >
              ...
              <Text style={styles(theme).heroLocation} numberOfLines={1}>
                {event.venue}
              </Text>
            </View>
            <View
...
export default function DashboardScreen() {
    ...
    if (PRIVATE_CODE_REGEX.test(trimmed)) {
      ...
      codeSearchTimeout.current = setTimeout(async () => {
        ...
        try {
          ...
          if (res.ok) {
            setPrivateCodeEvent(await res.json());
            setPrivateCodeError("");
          } else {
            setPrivateCodeEvent(null);
  ...
  const hasActiveFilters =
    ...
    [filters.date, filters.price, filters.sort].some(
      (v, i) => v !== ["all", "all", "soonest"][i],
    );
  ...
  const applyFilters = (events) =>
    events.filter((event) => {
      ...
      if (activeCategory === "Near Me") {
        ...
        return (
          (userCountry &&
            (country.includes(userCountry.toLowerCase()) ||
              venue.includes(userCountry.toLowerCase()))) ||
          (userUniversity &&
            (uni.includes(userUniversity.toLowerCase()) ||
              venue.includes(userUniversity.toLowerCase())))
        );
      }
  ...
  const matchesInterests = (e) =>
    userInterests.length > 0 &&
    e.category &&
    userInterests.includes(e.category);
  const sortByInterests = (events) => [
    ...events.filter(matchesInterests),
    ...events.filter((e) => !matchesInterests(e)),
  ];
    ...
    <Screen padded>
      ...
      <View style={S.searchRow}>
        ...
        <Pressable
          ...
          style={[
            S.filterBtn,
            hasActiveFilters && { backgroundColor: theme.colors.brand },
          ]}
        ...
        <View style={S.section}>
          ...
          {fetchingUsers ? (
            ...
            <EmptyState
              ...
              subtitle={`No results for "${keyword.slice(1)}"`}
            />
          ) : (
            <View style={{ gap: 10 }}>
              {userResults.map((user) => {
                ...
                return (
                  <Pressable
                    ...
                    onPress={() =>
                      router.push(`/users/u/${user._id || user.user_id}`)
                    }
                    style={[
                      S.userRow,
                      {
        ...
        <View style={S.section}>
          <View style={S.sectionHeader}>
            ...
            <View style={S.countPill}>
              <Text style={S.countPillText}>{trendingEvents.length}</Text>
            </View>
          </View>
        ...
        activeCategory !== "Near Me" && (
          <View style={S.section}>
            ...
            {premiumEvents.length === 0 ? (
              <EmptyState
                emoji="⭐"
                title="No premium events yet"
                subtitle="They'll show up here when they're live."
              />
            ) : (
              <ScrollView
                horizontal
            ...
            <EmptyState
              ...
              subtitle={
                keyword
                  ...
                  : "Check back later — events are coming!"
              }
            />
          ) : (
            <View style={{ gap: 6 }}>
