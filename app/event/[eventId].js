...
function TicketCard({ ticket, onSelect, selected, theme }) {
  ...
  const isSoldOut =
    ticket.capacity !== undefined && ticket.sold >= ticket.capacity;
  ...
  return (
    ...
    >
      <View style={styles.ticketCardLeft}>
        ...
        {ticket.description ? (
          <Text
            style={[styles.ticketDesc, { color: theme.colors.textMuted }]}
            numberOfLines={2}
          >
            {ticket.description}
          </Text>
        ...
        {isSoldOut ? (
          <Text style={[styles.soldOutLabel, { color: theme.colors.error }]}>
            Sold out
          </Text>
        ) : ticket.capacity !== undefined ? (
          <Text
            style={[styles.ticketCapacity, { color: theme.colors.textSubtle }]}
          >
      ...
      <View style={styles.ticketCardRight}>
        <Text
          style={[
            styles.ticketPrice,
            { color: isFree ? theme.colors.brand : theme.colors.text },
          ]}
        >
...
function VenueMap({ venue, lat, lng, theme }) {
  const [coords, setCoords] = useState(lat && lng ? { lat, lng } : null);
  const [mapLoading, setMapLoading] = useState(!lat || !lng);
  ...
  useEffect(() => {
    ...
    fetch(
      ...
      { headers: { "User-Agent": "UniHub/1.0" } },
    )
      .then((r) => r.json())
      .then((data) => {
        if (data.length > 0) setCoords({ lat: data[0].lat, lng: data[0].lon });
      })
      .catch(() => {})
  ...
  const mapUrl = coords
    ? `https://staticmap.openstreetmap.de/staticmap.php?center=${coords.lat},${coords.lng}&zoom=15&size=600x200&markers=${coords.lat},${coords.lng},red`
    : null;
  ...
  return (
    <TouchableOpacity
      onPress={openMaps}
      activeOpacity={0.85}
      style={[vmStyles.container, { borderColor: theme.colors.border }]}
    >
      {mapLoading ? (
        <View
          style={[
        ...
        >
          <MapPin size={26} color={theme.colors.textSubtle} />
        </View>
      )}
      <View
        style={[vmStyles.footer, { backgroundColor: theme.colors.surface }]}
      >
        <MapPin size={13} color={theme.colors.brand} />
        <Text
          style={[vmStyles.footerText, { color: theme.colors.text }]}
          numberOfLines={1}
        >
          {venue}
        </Text>
        <Text style={[vmStyles.footerCta, { color: theme.colors.brand }]}>
          Open →
        </Text>
      </View>
    </TouchableOpacity>
...
export default function EventDetailScreen() {
      ...
      if (res.ok) {
        ...
        if (data.ownerId) {
          fetch(`${API_URL}/user/details`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ user_token: data.ownerId }),
          })
            .then((r) => (r.ok ? r.json() : null))
            .then((owner) => {
              if (owner?._id) setOwnerUserId(owner._id);
            })
            .catch(() => {});
        }
        ...
        if (data.feedback?.length > 0) {
          ...
          const avg =
            data.feedback.reduce((s, f) => s + (f.rating || 0), 0) /
  ...
  const handleJoinWaitlist = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        Alert.alert(
          "Waitlist",
          data.msg || "Could not join waitlist. Try again.",
        );
      }
    } catch {
  ...
  if (loading)
    return (
      <View
        style={{
          flex: 1,
          backgroundColor: theme?.colors?.background || "#FAF9F0",
        }}
      >
        <SkeletonLoader
          variant="card"
          count={1}
          style={{ height: HERO_HEIGHT, borderRadius: 0, margin: 0 }}
        />
  ...
  const selectedPrice = selectedTicket
    ...
    : minimumPrice;
  const displayPrice =
    selectedPrice === 0 ? "Free" : `₦${selectedPrice.toLocaleString()}`;
  ...
  const getCtaLabel = () => {
    ...
    return selectedPrice === 0 ? "Register Free" : "Buy Now";
  };
  ...
  return (
    <View style={{ flex: 1, backgroundColor: theme.colors.background }}>
      ...
      >
        {/* Hero */}
        <View
          style={[styles.hero, { height: HERO_HEIGHT, width: SCREEN_WIDTH }]}
        >
          {event.cover || event.profile ? (
            <Image
              source={{ uri: event.cover || event.profile }}
              style={{
                width: SCREEN_WIDTH,
                height: HERO_HEIGHT,
                position: "absolute",
          ...
          {/* Back + Share row */}
          <View
            style={[styles.heroTopRow, { paddingTop: STATUS_BAR_HEIGHT + 12 }]}
          >
            <Pressable
              onPress={() => router.back()}
            ...
            <View style={styles.heroBadgeRow}>
              ...
              {event.visibility === "private" && (
                ...
                >
                  ...
                  <Text style={[styles.heroBadgeText, { color: "#C8E630" }]}>
                    PRIVATE
                  </Text>
                </View>
              )}
        ...
        <View style={styles.content}>
          ...
          {event.organizer && (
            <Pressable
              ...
              onPress={() => {
                if (ownerUserId) router.push(`/users/u/${ownerUserId}`);
              }}
            >
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text
                  style={[styles.organizerName, { color: theme.colors.text }]}
                >
                  {event.organizer}
                </Text>
              </View>
          ...
          >
            <View style={styles.infoRow}>
              ...
              <View style={{ flex: 1 }}>
                <Text
                  style={[styles.infoLabel, { color: theme.colors.textSubtle }]}
                >
                  Location
                </Text>
                <Text style={[styles.infoValue, { color: theme.colors.text }]}>
                  {event.hideLocation && !isRegistered
                    ? "Location revealed after registration"
                    : event.venue || "TBA"}
                </Text>
                {event.address && isRegistered && (
                  <Text
                    style={[styles.infoSub, { color: theme.colors.textMuted }]}
                  >
                    {event.address}
                  </Text>
                )}
            ...
            {!(event.hideLocation && !isRegistered) && event.venue && (
              <VenueMap
                venue={event.venue}
                lat={event.lat}
                lng={event.lng}
          ...
          {event.description && (
            <View style={styles.section}>
              ...
              <Text
                style={[styles.descText, { color: theme.colors.textMuted }]}
              >
                {event.description}
              </Text>
            </View>
          ...
          {event.requiresApproval && !isRegistered && (
            ...
            >
              ...
              <Text style={[styles.noticeText, { color: "#f59e0b" }]}>
                This event requires host approval. You may be added to a
                waitlist pending review.
              </Text>
            </View>
          )}
            ...
            <View
              style={[
                styles.noticeCard,
                {
                  ...
                  borderColor: "rgba(61,158,74,0.3)",
                },
              ]}
            >
              <CheckCircle size={16} color={theme.colors.success} />
              <Text
                style={[styles.noticeText, { color: theme.colors.success }]}
              >
                  ...
                  [
                    ...
                    },
                    {
                      ...
                      onPress: async () => {
                        ...
                        setReminderLoading(false);
                        if (!r.ok)
                          Alert.alert("Reminder", "Could not set reminder.");
                      },
                    },
                    {
                      text: "1 day before",
            ...
            >
              ...
              <Text
                style={[
                  ...
                  { color: reminderSet ? "#6366f1" : theme.colors.textMuted },
                ]}
              >
          ...
          {isRegistered && (
            ...
            >
              <Calendar
                size={16}
                color={
                  calendarAdded ? theme.colors.success : theme.colors.textMuted
                }
              />
              <Text
                style={[
          ...
          {(event.participants?.length || 0) > 0 && (
            ...
            >
              ...
              <Text
                style={[styles.noticeText, { color: theme.colors.textMuted }]}
              >
                <Text
                  style={{
                ...
                {event.participants.length === 1 ? "person is" : "people are"}{" "}
                attending
              </Text>
            </View>
          )}
          ...
          {avgRating && (
            <View style={styles.section}>
              ...
              >
                <Text style={[styles.reviewAvg, { color: theme.colors.text }]}>
                  {avgRating}
                </Text>
                <Text style={[styles.reviewStars, { color: "#F59E0B" }]}>
                ...
                >
                  {event.feedback?.length || 0} review
                  {(event.feedback?.length || 0) !== 1 ? "s" : ""}
                </Text>
              </View>
              {reviews.map((r, i) => (
                <View
                ...
                >
                  <View style={styles.reviewCardTop}>
                    <Text
                      style={[styles.reviewCardStars, { color: "#F59E0B" }]}
                    >
                      {"★".repeat(r.rating || 0)}
                    </Text>
                    <Text
                      style={[
                        styles.reviewCardDate,
                        { color: theme.colors.textSubtle },
                      ]}
                    >
                      {r.createdAt
      ...
      >
        {!isOwner && (
          <View style={styles.ctaLeft}>
            <Text style={[styles.ctaPrice, { color: theme.colors.brand }]}>
              {displayPrice}
            </Text>
            <Text
              style={[styles.ctaPriceSub, { color: theme.colors.textSubtle }]}
            >
              {selectedTicket?.name || "per ticket"}
            </Text>
          </View>
        )}
        <Pressable
          style={[
            styles.ctaBtn,
            {
              backgroundColor: ctaDisabled
                ? theme.colors.surfaceMuted
                : theme.colors.brand,
              opacity: waitlistLoading ? 0.75 : 1,
              flex: isOwner ? 1 : undefined,
            },
          ]}
          onPress={
            isOwner
        ...
        >
          {waitlistLoading ? (
            ...
            <CheckCircle
              size={18}
              color={
                ctaDisabled ? theme.colors.textSubtle : theme.colors.textOnBrand
              }
            />
          ) : (
            <Ticket
